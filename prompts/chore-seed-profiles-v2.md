...
- The 12 seed avatar SVG files must be supplied at a known local path before running —
  ...
  path
- `ts-node` is installed under `functions/node_modules/`, not the repo root — verify
  this and write any run instructions accordingly (`cd functions && npx ts-node
  ../scripts/seedBetaProfiles.ts <dir>`), not a root-relative `npx ts-node
  scripts/seedBetaProfiles.ts <dir>` invocation, which will fail to resolve `ts-node`
  from repo root
...
//
// (Verified working command — confirmed against this repo's actual tooling location,
// not assumed from a generic root-level ts-node setup.)
//
...
- [ ] Running `npx ts-node ../scripts/seedBetaProfiles.ts <avatar-dir>` from inside
      `functions/` creates 12 Auth users and 12 corresponding `/users/{uid}` Firestore
      documents
